...
class GenerationMixin:
    ...
    def generate(
        ...
        **kwargs,
        ...
        # generation_config是标准的模型生成参数
        # model_kwargs是在此基础上，融合了用户自己额外的参数
        ...
        elif generation_mode == GenerationMode.LFD_GENERATION:
            if self._is_stateful:
                # LFD decoding was not designed for stateful models, and would require some changes
                raise ValueError(
                    f"lfd decoding is not supported with stateful models, such as {self.__class__.__name__}"
                )
            result = self._lfd_decoding(
                input_ids,
                lfd_layers=generation_config.lfd_layers,
                logits_processor=prepared_logits_processor,
                stopping_criteria=prepared_stopping_criteria,
                generation_config=generation_config,
                synced_gpus=synced_gpus,
    ...
    def _lfd_decoding(
        ...
        **model_kwargs,
        ...
        if self.config.is_encoder_decoder:
            raise ValueError("LFD decoding is only available for decoder-only models.")
        # init values
        ...
        premature_layer_dist = {l:0 for l in lfd_layers}
        ...
        # prepare layers for LFD decoding
        final_layer = self.config.get_text_config().num_hidden_layers

        # Set the `lfd_layers` to a list of integers for layer indices to contrast manually specified layers.
        if isinstance(lfd_layers, list):
            candidate_premature_layers = [i for i in lfd_layers if i < final_layer]
        else:
            raise ValueError("lfd_layers must be a list of integers.")

        lm_head = self.get_output_embeddings()
        if lm_head is None:
            raise ValueError("LFD is not supported for models that don't have output embeddings.")
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            outputs = self(
                **model_inputs,
                return_dict=True,
                output_last_token_states=True
            )
            # logits.shape: (batch, seq, vocab)
            # hidden_states.shape: [layer](batch, seq, dim)
            # attentions.shape: [layer](batch, num_heads, seq, seq)
            # last_token_hidden_states: [layer](batch, dim)
            # last_token_residual_states: [layer](batch, dim)

            # .float() is needed to retain precision for later logits manipulations
            final_layer_next_token_logits = outputs.logits[:, -1, :].detach().to(copy=True, dtype=torch.float32)
            final_logits = outputs.logits[:, -1, :].float()
            candidate_premature_logits = {}
            candidate_premature_norm_logits = {}
            candidate_premature_norm_residual_logits = {}
            for candidate_premature_layer in candidate_premature_layers:
                candidate_premature_logits[candidate_premature_layer] = lm_head(
                    outputs.last_token_hidden_states[candidate_premature_layer-1]
                ).to(final_logits.device)
                candidate_premature_norm_logits[candidate_premature_layer] = lm_head(
                    outputs.last_token_norm_hidden_states[candidate_premature_layer-1]
                ).to(final_logits.device)
                candidate_premature_norm_residual_logits[candidate_premature_layer] = lm_head(
                    outputs.last_token_norm_residual_states[candidate_premature_layer-1]
                ).to(final_logits.device)
            ...
            next_token_logits, premature_layer = _lfd_select(
...
def _lfd_select(
    ...
    candidate_premature_norm_logits: Dict[int, torch.FloatTensor],
    candidate_premature_norm_residual_logits: Dict[int, torch.FloatTensor],
    ...
    # shape: (cand_layer, batch, vocab)
    stacked_premature_logits = torch.stack([candidate_premature_norm_logits[i].float() for i in candidate_premature_layers], dim=0)
    stacked_premature_residual_logits = torch.stack([candidate_premature_norm_residual_logits[i].float() for i in candidate_premature_layers], dim=0)

    # 2. Calculate the softmax values for mature_layer and all premature_layers
    # shape: (cand_layer, batch, vocab)
    softmax_premature_logits = F.softmax(stacked_premature_logits, dim=-1)
    softmax_premature_residual_logits = F.softmax(stacked_premature_residual_logits, dim=-1)

    # 3. Calculate the average distribution
    # shape: (cand_layer, batch, vocab)
    avg_dist = 0.5 * (softmax_premature_logits + softmax_premature_residual_logits)

    # 4. Calculate log-softmax for the KL divergence
    # shape: (cand_layer, batch, vocab)
    log_softmax_premature_logits = F.log_softmax(stacked_premature_logits, dim=-1)
    # shape: (cand_layer, batch, vocab)
    log_softmax_premature_residual_logits = F.log_softmax(stacked_premature_residual_logits, dim=-1)

    # 5. Calculate the KL divergences and then the JS divergences
    # shape: (cand_layer, batch)
    kl1 = F.kl_div(log_softmax_premature_logits, avg_dist, reduction="none").mean(-1)
    # shape: (cand_layer, batch)
    kl2 = F.kl_div(log_softmax_premature_residual_logits, avg_dist, reduction="none").mean(-1)
    js_divs = 0.5 * (kl1 + kl2)

    # shape: (cand_layer,)
    js_divs = js_divs.mean(-1)

    premature_layer = candidate_premature_layers[int(js_divs.argmin().item())]

    # random
    # import random
    # premature_layer = random.choice(candidate_premature_layers)
