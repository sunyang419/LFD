...
class GenerationConfig(PushToHubMixin):
    ...
    for text-decoder, text-to-text, speech-to-text, and vision-to-text models:
        ...
        - *lfd decoding* if `lfd_layers` is passed to `.generate()`
    ...
    Arg:
        ...
        lfd_layers (`str` or `List[int]`, *optional*):
            The layers to use for LFD decoding. If `None`, LFD decoding is not used. 
    ...
    def __init__(self, **kwargs):
        ...
        self.lfd_layers = kwargs.pop("lfd_layers", None)
    ...
    def get_generation_mode(self, assistant_model: Optional["PreTrainedModel"] = None) -> GenerationMode:
        ...
        # LFD generation may extend some generation modes
        if self.lfd_layers is not None:
            if generation_mode in ("greedy_search", "sample"):
                generation_mode = GenerationMode.LFD_GENERATION
            else:
                raise ValueError(
                    "You've set `lfd_layers`, which triggers lfd generate. Currently, lfd generate "
                    "is only supported with Greedy Search and Sample."
                )
